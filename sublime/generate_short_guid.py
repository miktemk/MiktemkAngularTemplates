...
class GenerateShortGuidCommand(sublime_plugin.TextCommand):
    """
    ...
    def run(self, edit, single = False):
        for r, value in zip(self.view.sel(), self.generateUuids(True, single)):
            self.view.replace(edit, r, value)

    def generateUuids(self, short, single):
        settings = sublime.load_settings('Preferences.sublime-settings')
        uppercase = settings.get('generate_short_guid_uppercase')
